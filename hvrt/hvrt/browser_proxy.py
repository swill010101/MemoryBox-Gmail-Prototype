...
the review player stays black / "Waiting for video frame size…". VLC still
shows the full frame. This builds a H.264 proxy under working/browser_proxies/.
"""
...
class BrowserProxyManager:
    ...
    def proxy_path(self, video_id: int) -> Path:
        return self.proxy_dir / f"{int(video_id)}.mp4"

    def has_ready_proxy(self, video_id: int, source: Path) -> bool:
        dest = self.proxy_path(video_id)
    ...
    def status(self, video_id: int, source: Path | None = None) -> dict[str, Any]:
        vid = int(video_id)
        with self._lock:
        ...
        if ready and job.get("status") not in ("running", "queued"):
            return {
                ...
                "stream_url": f"/api/media/{vid}?proxy=1",
            }
        if not job:
            return {
                "video_id": vid,
        ...
        job["video_id"] = vid
        job["stream_url"] = f"/api/media/{vid}?proxy=1"
        return job

    def start(self, video_id: int, source: Path) -> dict[str, Any]:
        vid = int(video_id)
        source = Path(source)
        ...
        t = threading.Thread(
            target=self._run, args=(vid, source), daemon=True, name=f"browser-proxy-{vid}"
        )
        t.start()
        return self.status(vid, source)

    def _append(self, video_id: int, line: str) -> None:
        line = (line or "").rstrip()
        if not line:
            return
        with self._lock:
            job = self._jobs.setdefault(video_id, {})
            log = list(job.get("log") or [])
            log.append(line)
            job["log"] = log[-80:]

    def _set(self, video_id: int, **kwargs: Any) -> None:
        with self._lock:
            job = self._jobs.setdefault(video_id, {})
            job.update(kwargs)
    ...
    def _run(self, video_id: int, source: Path) -> None:
        dest = self.proxy_path(video_id)
        ...
        self._append(video_id, f"source codec: {codec}")
        self._append(video_id, f"ffmpeg → {dest}")
        cmd = [
            ffmpeg,
            "-y",
            "-i",
        ...
        try:
            ...
            for line in proc.stdout:
                self._append(video_id, line)
                # crude progress bump while ffmpeg runs
            ...
            self._append(video_id, f"ERROR: {e}")
