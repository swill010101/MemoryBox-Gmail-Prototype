...
def units_from_pack(pack: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for u in pack.get("units") or []:
        if not isinstance(u, dict):
            continue
        eid = (
            str((u.get("provenance") or {}).get("evidence_id") or "")
        ...
        out.append(
            {
                ...
                "kind": u.get("kind"),
                "source_type": u.get("source_type") or (u.get("normalization") or {}).get("source_type"),
                "time": str(time_raw or "")[:32],
                "people": u.get("people") or [],
                "place": u.get("place"),
                "content": str(u.get("content") or u.get("title") or "")[:800],
...
def in_scope_ids(pack: dict[str, Any]) -> set[str]:
    ...
    for u in units_from_pack(pack):
        ...
        for k in ("evidence_id", "external_id", "story_id", "journal_id", "artifact_id"):
