...
- If an I1/HVRT Peggy clip was moved out of `P:\photos\home videos`, prove cannot reverse `vid-` hashes to a filename. Check `meta.immich_confirmed_asset.originalFileName` / `originalPath`, and `meta.legacy_hvrt_clips`. Put that file back under Home Videos, then `.\startmb.cmd -Restart`.
