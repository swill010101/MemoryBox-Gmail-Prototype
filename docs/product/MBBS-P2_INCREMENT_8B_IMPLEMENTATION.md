...
- Acceptance corpus: negative video + both-people video in harness; FlightSim scans HVRT inventory under `MEMORYBOX_VIDEO_MEDIA_ROOT` (`P:\photos\home videos` via `startmb` / `config/video_worker.env`) **and** Immich VIDEO assets for the Person (Ask video filter / encoded-video on disk). Default positive clip: `deb5c1f8-4d01-457c-9637-185268e4b820` (override with `MEMORYBOX_P2_I8B_POSITIVE_ASSET_ID` or moment id). FlightSim prove requires at least one `mb_native_i8b` appearance range.
