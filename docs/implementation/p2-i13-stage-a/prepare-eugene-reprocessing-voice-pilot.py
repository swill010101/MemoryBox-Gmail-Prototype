"""Read-only preflight for the bounded Eugene reprocessing lifecycle proposal; no media or database writes."""
...
PROPOSAL = ROOT / 'docs/implementation/p2-i13-stage-a/eugene-affected-reprocessing-proposal.json'
...
def validate_rows(proposal: dict, rows: list[dict]) -> dict:
    ...
    fresh_keys = {proposal['selections'][0]['key'], proposal['selections'][1]['key']}
    for annotation_id, span in expected.items():
        ...
        if span['key'] in fresh_keys:
            if row['person_id'] != TARGET_PERSON_ID or row['speaker_state'] != 'person' or row['pilot_uses']:
                raise RuntimeError('Grandpa sessions 2 002 Eugene evidence is no longer fresh and active.')
        elif span['key'] == 'T2-as-Eugene-negative':
