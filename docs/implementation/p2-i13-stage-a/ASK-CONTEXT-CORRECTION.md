...
3. Run `python -B -m unittest discover -s tests -p test_i13_stage_a.py -v` (33 tests), then `python -B -m unittest discover -s tests -p test_i13_ask_context.py -v` (18 tests). Node is required for the Stage A suite. Run `node --check memorybox/explore/static/explore.js`.
...
## Curator progress correction

Tom reports the other current checks working and Done appearing as a new Ask begins. Confirmed code cause: session-keyed polling could read the prior request's final status, including while a fresh named search rotated the session. The Find POST and progress GET now share a per-request progress ID. A request-local context carries the ID through existing progress writers; completed/failed scoped entries are removed. The browser ignores Done while the result is pending, and an older Find's cleanup cannot stop a newer poll. Legacy session-based progress remains available to existing callers. No extra provider work or polling frequency increase.

Validation: 18 context/provider/progress tests plus 33 Stage A tests (51), JavaScript syntax and actual browser component proofs for stale Done, late poll response rejection, current status display and matching Find progress ID. Proof: browser-progress-proof.json/png. No live runtime access, processing, migrations or I12 edits. Gate 2 step 14 remains open only for the reported curator defect within these current Ask checks; full I13 acceptance and previously deferred playback/learning remain separate. On FlightSim, run consecutive named and follow-up Asks: each should begin with Collecting photos, display only that request's progress, then show results. Confirm no premature Done and report completion for step 15 review.

## Live verification received - 2026-09-05

After the deployment instructions for 6d1bf63678d79bd021bc4803eca09f0211a35ca1 and the requested two-consecutive-Ask curator check, Tom reported "passes". Earlier owner reports confirmed the photo Gallery and location-chip correction working. Record the current Ask/Gallery correction checks as passed by owner report. This is not an independent runtime inspection, measured latency result, or authorization for processing. The deployed code reference remains 6d1bf63678d79bd021bc4803eca09f0211a35ca1; this documentation-only update requires no redeployment. Gate 2 step 15 review is next. Source-playback limitations, fragment presentation, annotations and face/voice recognition acceptance remain separate open work.
