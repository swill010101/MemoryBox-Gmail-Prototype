...
3. Run `python -B -m unittest discover -s tests -p test_i13_stage_a.py -v` (33 tests), then `python -B -m unittest discover -s tests -p test_i13_ask_context.py -v` (12 tests). Node is required for the Stage A suite. Run `node --check memorybox/explore/static/explore.js`.
