...
try {
    ...
    if (-not (Test-Path -LiteralPath $plan)) { throw 'Reviewed N1 Unknown pilot plan is missing.' }
    $preview = & $python -B -m memorybox.processing.control preview --plan $plan
    Require-LastExit 'N1 Unknown pilot plan preview failed.'
    $previewJson = (($preview | Where-Object { $_ -and $_.Trim() }) -join "`n") | ConvertFrom-Json
    if ($previewJson.purpose -ne 'voice_pilot' -or $previewJson.work_items -ne 4 -or $previewJson.max_attempts -ne 4 -or $previewJson.audio_seconds -ne 38.46) { throw 'N1 Unknown pilot preview did not preserve the fixed scope.' }
    if ($previewJson.plan_sha256 -ne $ExpectedPlanSha.ToLowerInvariant()) { throw 'Reviewed N1 Unknown pilot plan hash mismatch.' }
    if (-not $Execute) {
    ...
    $backupDir = Join-Path $backupRoot "i13-final-pre-n1-unknown-$token"
    $remoteDir = "/tmp/mb-i13-final-pre-n1-unknown-$token"
    ...
    Require-LastExit 'N1 Unknown pilot admission registration failed.'
    ...
    try {
        ...
        Require-LastExit 'N1 Unknown pilot admission start failed.'
        ...
        Require-LastExit 'N1 Unknown pilot run failed; no automatic retry.'
    ...
    if ($after.voice_admissions -ne ($before.voice_admissions + 1) -or $after.voice_runs -ne ($before.voice_runs + 1) -or $after.voice_attempts -ne ($before.voice_attempts + 4) -or $after.voice_events -lt ($before.voice_events + 2) -or $after.voice_results -ne ($before.voice_results + 1)) { throw 'Final N1 Unknown pilot verification failed.' }
