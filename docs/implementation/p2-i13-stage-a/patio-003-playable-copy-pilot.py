"""Single pinned-source Patio 003 normalization pilot. Default: read-only check. No MB imports.
...
VID = "vid-8163a680131fd30a"
SOURCE_HASH = "ccda7f166a780c512fa9dc72b366ad20b314003aa6cc2f3869a0cf5396dc37e1"
SOURCE_SIZE = 1055204616
DURATION = 931.931
VIDEO_DURATION = 931.931
AUDIO_DURATION = 931.931
MAX_BYTES = 4 * 1024**3
MIN_FREE = 10 * 1024**3
WALL_SECONDS = 600
...
DEST_REL = "browser_proxies/28fafdd904eda5f82617c328.mp4"
ATTEMPT_REL = "i13-playable-pilot-8163a680131fd30a-v01"
...
def validate_streams(data, output=False):
    """Validate the pinned MPEG-4 source or H.264 staged copy."""
    streams=data.get('streams',[]); videos=[x for x in streams if x.get('codec_type')=='video']; audios=[x for x in streams if x.get('codec_type')=='audio']
    if len(videos)!=1 or len(audios)!=1 or len(streams)!=2: raise RuntimeError('Expected exactly one video and one audio stream')
    video,audio=videos[0],audios[0]
    if (video.get('codec_name') != ('h264' if output else 'mpeg4')): raise RuntimeError('Unexpected video codec')
    if video.get('pix_fmt') not in {'yuv420p','yuvj420p'}: raise RuntimeError('Unexpected video pixel format')
    if output and video.get('profile') != 'High': raise RuntimeError('Unexpected staged H.264 profile')
    if audio.get('codec_name')!='aac': raise RuntimeError('Unexpected audio codec')
    for row in (data.get('format',{}),video,audio):
        duration=float(row.get('duration','nan')); start=float(row.get('start_time','nan'))
        if not math.isfinite(duration) or abs(duration-DURATION)>0.5: raise RuntimeError('Duration mismatch')
        if not math.isfinite(start) or abs(start)>0.05: raise RuntimeError('Source-zero timeline mismatch')
