...
$plan = Join-Path $release 'i13-reviewed-tom-voice-pilot-plan.json'
...
try {
    ...
    if (-not $Execute) {
        [pscustomobject]@{ ok=$true; mode='check_only'; release_sha=$ExpectedReleaseSha.ToLowerInvariant(); migration='032_p2_i13_voice_pilot.sql already present'; private_audio_processed=$false; database_writes=$false; admission_created=$false } | ConvertTo-Json
        exit 0
    }
    ...
    if (Test-Path -LiteralPath $plan) { throw 'Tom pilot plan path already exists; preserve it and stop.' }

    $token = [guid]::NewGuid().ToString('N')
    $backupDir = Join-Path $backupRoot "i13-final-pre-tom-$token"
    $remoteDir = "/tmp/mb-i13-final-pre-tom-$token"
    ...
    $planSha = (($prepared | Where-Object { $_ -and $_.Trim() }) -join "`n" | ConvertFrom-Json).plan_sha256
    if (-not $planSha) { throw 'Prepared plan digest is missing.' }
    & $python -B -m memorybox.processing.control preview --plan $plan | Out-Null
    ...
    Require-LastExit 'Tom pilot admission registration failed.'
    ...
    try {
        ...
        Require-LastExit 'Tom pilot admission start failed.'
        ...
        Require-LastExit 'Tom pilot run failed; no automatic retry.'
    ...
    if ($after.voice_admissions -ne ($before.voice_admissions + 1) -or $after.voice_runs -ne ($before.voice_runs + 1) -or $after.voice_attempts -ne ($before.voice_attempts + 4) -or $after.voice_events -lt ($before.voice_events + 2) -or $after.voice_results -ne ($before.voice_results + 1)) { throw 'Final Tom pilot verification failed.' }
    [pscustomobject]@{ ok=$true; release_sha=$ExpectedReleaseSha.ToLowerInvariant(); admission_id=$admissionId; plan_sha256=$planSha; backup_file=$backupFile; backup_sha256=$backupHash; results=$runResult.results; legacy_counts_unchanged=$true; private_audio_processed=$true; automatic_retry=$false } | ConvertTo-Json -Depth 8
}
finally {
    Pop-Location
}