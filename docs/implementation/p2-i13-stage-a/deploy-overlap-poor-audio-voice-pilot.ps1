...
$plan = Join-Path $release 'i13-reviewed-overlap-poor-audio-voice-pilot-plan.json'
...
try {
    ...
    if (-not (Test-Path -LiteralPath $plan)) { throw 'Reviewed overlap pilot plan is missing.' }
    ...
    $preview = & $python -B -m memorybox.processing.control preview --plan $plan 2>&1
    ...
    if ($previewJson.purpose -ne 'voice_pilot' -or $previewJson.work_items -ne 4 -or $previewJson.max_attempts -ne 4 -or $previewJson.audio_seconds -ne 32.38) { throw 'Overlap pilot preview did not preserve the fixed scope.' }
    if ($previewJson.plan_sha256 -ne $ExpectedPlanSha.ToLowerInvariant()) { throw 'Reviewed overlap pilot plan hash mismatch.' }
    if (-not $Execute) {
    ...
    if ($before.tom_current -or $before.n1_current -or $before.patio_current -or $before.reprocessing_current) {
        throw 'Current Tom, N1, Patio, or reprocessing pilot results must remain current before this overlap run.'
    }
    $token = [guid]::NewGuid().ToString('N')
    $backupDir = Join-Path $backupRoot "i13-final-pre-overlap-poor-audio-$token"
    $remoteDir = "/tmp/mb-i13-final-pre-overlap-poor-audio-$token"
    ...
    $registered = & $python -B -m memorybox.processing.control register --plan $plan --review-ref $ApprovalReference 2>&1
    Require-LastExit 'Overlap pilot admission registration failed.' $registered
    ...
    try {
        & $python -B -m memorybox.processing.control start --id $admissionId --reference $ApprovalReference 2>&1 | Out-Null
        Require-LastExit 'Overlap pilot admission start failed.' @()
        ...
        Require-LastExit 'Overlap pilot run failed; no automatic retry.' $run
        $runResult = (($run | Where-Object { $_ -and $_.Trim() }) -join "`n") | ConvertFrom-Json
    }
    finally {
        & $python -B -m memorybox.processing.control stop --id $admissionId --reference $ApprovalReference 2>&1 | Out-Null
