...
$plan = Join-Path $release 'i13-reviewed-eugene-patio-voice-pilot-plan.json'
...
try {
    ...
    $expectedPlanRelative = 'i13-reviewed-eugene-patio-voice-pilot-plan.json'
    ...
    $backupDir = Join-Path $backupRoot "i13-final-pre-eugene-patio-$token"
    ...
    if ($after.voice_admissions -ne ($before.voice_admissions + 1) -or $after.voice_runs -ne ($before.voice_runs + 1) -or $after.voice_attempts -ne ($before.voice_attempts + 4) -or $after.voice_events -lt ($before.voice_events + 2) -or $after.voice_results -ne ($before.voice_results + 1)) { throw 'Final Eugene Patio pilot verification failed.' }
