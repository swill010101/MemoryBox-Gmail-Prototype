...
$selection = Join-Path $PSScriptRoot 'eugene-affected-reprocessing-proposal.json'
...
$writtenPlan = Join-Path $release 'i13-reviewed-eugene-reprocessing-voice-pilot-plan.json'
$tempPlan = $null
...
try {
    ...
    if (-not (Test-Path -LiteralPath $selection)) { throw 'Eugene reprocessing pilot selection is missing.' }
    ...
    if ($WritePlan) {
        if (Test-Path -LiteralPath $writtenPlan) { throw 'Eugene reprocessing pilot plan path already exists; preserve it and stop.' }
        $outputPlan = $writtenPlan
    }
    else {
        $tempPlan = Join-Path ([System.IO.Path]::GetTempPath()) ("mb-i13-eugene-reprocessing-plan-$([guid]::NewGuid().ToString('N')).json")
        $outputPlan = $tempPlan
    }
    ...
    Require-LastExit 'Eugene reprocessing pilot plan preparation failed.' $prepared
    ...
    if ($previewJson.purpose -ne 'voice_pilot' -or $previewJson.work_items -ne 4 -or $previewJson.max_attempts -ne 4 -or $previewJson.audio_seconds -ne 39.52) {
        throw 'Eugene reprocessing pilot preview did not preserve the fixed bounded scope.'
    }
