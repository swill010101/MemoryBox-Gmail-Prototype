...
- [ ] Preserve independent face and voice evidence and show corroboration transparently when both exist. **Implementation complete (read-only):** [FACE-VOICE-CORROBORATION-NOTE.md](FACE-VOICE-CORROBORATION-NOTE.md), `GET /review/face-voice-corroboration`, Review UI section.
- [ ] Review poor-audio, multiple-person and no-match cases against owner-confirmed annotations; uncertain output must not masquerade as confirmed identity. Overlap pilot `339b3a14…` measured E2 Eugene match at `0.584` with TV background and O2/N1 no-match controls; narrow interval-specific evidence only.
...
- [x] Overlap/poor-audio bounded pilot completed: admission `339b3a14-5069-4554-846f-dc84d6745c00` on 1532 interval `bbceb696` matched Eugene at `0.584` with O2 off-camera Tom and N1 TV no-match controls; prior Tom/N1/Patio/reprocessing evidence remained current. See [overlap-poor-audio-voice-pilot-report.json](overlap-poor-audio-voice-pilot-report.json). Face/voice corroboration and generalized accuracy remain unproven.
## Current evidence
