...
## Owner-reported annotation interaction defects (2026-09-06)

Tom reported that the annotation heading scrolls out of view, Person selection disappears on Save, confirmation is absent, reopened history appears empty, and correction-field keys navigate between videos. The authorized correction keeps controls discoverable, isolates editing keys, preserves selection during form interaction, and shows durable save/error feedback and history. Live persistence of the reported assignment is unverified; no runtime writes or learning are authorized by this UI repair. Tom separately confirmed live migration 031 committed with 1,736 snapshots, 140,441 archived words and zero mismatches.

The correction places annotation controls above the independently scrolling transcript, bounds the expanded panel, ignores form events during word selection, removes obsolete selection handlers after refresh, and prevents form keys from reaching viewer navigation. Save feedback and the saved Person survive the transcript reload; reopened history shows the saved count. API errors remain visible without clearing input. No persistence/schema/learning behavior changed.

Validation: 18 synthetic Chromium interaction checks pass using the actual transcript fetch/render and annotation handlers with mocked read/write responses. Coverage includes save/reopen, error retention, selected word IDs, form-key isolation, continued viewer navigation, reachable Save, and the heading remaining visible at the transcript bottom. Eight modal layout cases pass; 33 Stage A tests and three annotation contract tests pass (12 opt-in database tests skipped). JavaScript syntax passes. Synthetic browser screenshots were inspected; live save/persistence and correction acceptance still require Tom's FlightSim check. No runtime queries, migrations or processing ran during this repair.

Deploy as a fresh detached checkout of the correction commit using the existing start-fragment-release.ps1 -ExpectedSha helper. Migration 031 is already applied: do not repeat it or use the old preflight requiring pending 031. Close existing services normally, start the correction with -Start, refresh the browser, and verify a Person assignment, correction, saved confirmation and reopened history. Keep Learn untouched and processing locked.

## Saved assignment review follow-up

Tom reports a saved assignment count survives reopening and correction keys work, but the count only toggles the form and does not identify the Person or selected text. Add visible saved-assignment entries with speaker, source time range, text excerpt and an explicit Review assignment button. Review must populate the form and visibly mark the exact saved words without changing playback or writing data. Live evidence confirms the count, not yet the full reviewed annotation contents.

Implemented visible history on form expansion, speaker/time/text entries and explicit Review buttons. Review restores fields, highlights exact words, scrolls only the transcript and collapses history to expose the editor. Twenty synthetic Chromium interaction checks and JavaScript syntax pass; the rendered saved-entry screenshot was reviewed. Database writes and schema are unchanged. One browser cleanup timeout was resolved by graceful browser shutdown; its untracked temporary profile remains excluded from the commit. FlightSim review of the saved Person/span remains pending.
