...
$selection = Join-Path $PSScriptRoot 'tom-n1-unknown-no-match-pilot-proposal.json'
...
$writtenPlan = Join-Path $release 'i13-reviewed-n1-unknown-voice-pilot-plan.json'
$tempPlan = $null
...
try {
    ...
    if (-not (Test-Path -LiteralPath $selection)) { throw 'N1 Unknown pilot selection is missing.' }
    ...
    if ($WritePlan) {
        if (Test-Path -LiteralPath $writtenPlan) { throw 'N1 Unknown pilot plan path already exists; preserve it and stop.' }
        $outputPlan = $writtenPlan
    }
    else {
        $tempPlan = Join-Path ([System.IO.Path]::GetTempPath()) ("mb-i13-n1-unknown-plan-$([guid]::NewGuid().ToString('N')).json")
        $outputPlan = $tempPlan
    }
    ...
    Require-LastExit 'N1 Unknown pilot plan preparation failed.'
    ...
    if ($previewJson.purpose -ne 'voice_pilot' -or $previewJson.work_items -ne 4 -or $previewJson.max_attempts -ne 4) {
        throw 'N1 Unknown pilot preview did not preserve the fixed bounded scope.'
    }
